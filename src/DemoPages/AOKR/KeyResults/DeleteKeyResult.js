...
const DeleteKeyReslt = ({ classes, ...props }) => {
  const { addToast } = useToasts();

  const onDelete = (id) => {
    
    props.deletekeyResults(id, () =>
  ...
  let buttons = types.map((value, index) => {
    ...
    return (
      ...
      > 
        ...
        &nbsp;&nbsp;
        <Typography variant="inherit">Delete Key Result</Typography>
      </MenuItem>
    );
  });
  return (
    <Fragment>
      ...
      >
        ...
        >
          ...
          <ModalBody>
            <h3>
            Please confirm you wish to delete this Key Result :  {props.title}
            </h3>
          </ModalBody>
          <ModalFooter>
...
const mapActionToProps = {
  deletekeyResults: actions.DeleteKR,
};

export default connect(mapStateToProps, mapActionToProps)(DeleteKeyReslt);
